...
def get_args_parser():
    ...
    # image path
    parser.add_argument('--img_path', type=str, default='demo/vg5.jpg',
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for idx, ax_i, (sxmin, symin, sxmax, symax), (oxmin, oymin, oxmax, oymax) in \
            ...
            if vg_sub in verified_classes and vg_obj in verified_classes:
                imagenet_sub = NonCommon_processing_Resnet(x_min=sxmin, y_min=symin, x_max=sxmax, y_max=symax, image=im)
                imagenet_obj = NonCommon_processing_Resnet(x_min=oxmin, y_min=oymin, x_max=oxmax, y_max=oymax, image=im)
                ax.set_title(imagenet_sub+' '+REL_CLASSES[probas[idx].argmax()]+'\n'+imagenet_obj, fontsize=6)

            elif vg_sub not in verified_classes and vg_obj not in verified_classes:
                #sub_class = NonCommon_processing_Resnet(x_min=sxmin, y_min=symin, x_max=sxmax, y_max=symax, image=im)
                #obj_class = NonCommon_processing_Resnet(x_min=oxmin, y_min=oymin, x_max=oxmax, y_max=oymax, image=im)
                if vg_sub in PERSON_CLASSES:
                    vg_sub = person_processing(x_min=sxmin, y_min=symin, x_max=sxmax, y_max=symax, image=im, i=1)
                ax.set_title(vg_sub+' '+REL_CLASSES[probas[idx].argmax()]+'\n'+vg_obj, fontsize=6) 

            elif vg_sub in verified_classes and vg_obj not in verified_classes:
                imagenet_sub = NonCommon_processing_Resnet(x_min=sxmin, y_min=symin, x_max=sxmax, y_max=symax, image=im)
                #obj_class = NonCommon_processing_Resnet(x_min=oxmin, y_min=oymin, x_max=oxmax, y_max=oymax, image=im)
                ax.set_title(imagenet_sub+' '+REL_CLASSES[probas[idx].argmax()]+'\n'+vg_obj, fontsize=6)

            elif vg_sub not in verified_classes and vg_obj in verified_classes:
                #sub_class = NonCommon_processing_Resnet(x_min=sxmin, y_min=symin, x_max=sxmax, y_max=symax, image=im)
                if vg_sub in PERSON_CLASSES:
                    vg_sub = person_processing(x_min=sxmin, y_min=symin, x_max=sxmax, y_max=symax, image=im, i=1)
                imagenet_obj = NonCommon_processing_Resnet(x_min=oxmin, y_min=oymin, x_max=oxmax, y_max=oymax, image=im)
            ...
            #ax.set_title(sub_class+' '+REL_CLASSES[probas[idx].argmax()]+' '+CLASSES[probas_obj[idx].argmax()], fontsize=10)

        fig.tight_layout(pad=3.0, h_pad=1.0, w_pad=1.0)
        plt.savefig('person_dog.png')
        plt.show()
